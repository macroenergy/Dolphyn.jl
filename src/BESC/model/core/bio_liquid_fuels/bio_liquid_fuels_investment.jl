...
function bio_liquid_fuels_investment(EP::Model, inputs::Dict, setup::Dict)
	...
	@constraint(EP,cMaxCapacity_per_unit_BIO_LF[i = intersect(dfBioLF[dfBioLF.Max_capacity_tonne_per_hr.>0, :R_ID], 1:BIO_LF_RES_ALL)], EP[:vCapacity_BIO_LF_per_type][i] <= BIO_LF_Capacity_Max_Limit[i])
