...
function emissions_hsc(EP::Model, inputs::Dict, setup::Dict)
	...
	@expression(EP, eH2EmissionsByPlant[k=1:H,t=1:T], inputs["fuel_CO2"][dfH2Gen[!,:Fuel][k]]* dfH2Gen[!,:etaFuel_MMBtu_per_tonne][k]* EP[:vH2Gen][k,t]) 
    ...
    if (setup["H2CO2Cap"] ==4 && setup["SystemCO2Constraint"] ==1)
        ...
        @expression(EP,eCH2EmissionsPenaltybyZone[z=1:Z],
        sum(inputs["omega"][t]*sum(eH2EmissionsByZone[z,t]*inputs["dfH2CO2Price"][z,cap] for cap=findall(x->x==1, inputs["dfH2CO2CapZones"][z,:])) for t= 1:T)
        )
        # Sum over each policy type, each zone and each time step
        @expression(EP,eCH2EmissionsPenaltybyPolicy[cap=1:inputs["H2NCO2Cap"]],
        sum(inputs["omega"][t]*sum(eH2EmissionsByZone[z,t]*inputs["dfH2CO2Price"][z,cap] for z=findall(x->x==1, inputs["dfH2CO2CapZones"][:,cap])) for t= 1:T)
        )
        # Total emissions penalty across all policy constraints
        @expression(EP,eCH2GenTotalEmissionsPenalty,
        sum(eCH2EmissionsPenaltybyPolicy[cap] for cap=1:H2NCO2Cap))
    ...
    elseif (setup["CO2Cap"] ==4 && setup["SystemCO2Constraint"] ==2) 
        ...
        # Emissions penalty by zone - needed to report zonal cost breakdown
        @expression(EP,eCH2EmissionsPenaltybyZone[z=1:Z],
        sum(inputs["omega"][t]*sum(eH2EmissionsByZone[z,t]*inputs["dfCO2Price"][z,cap] for cap=findall(x->x==1, inputs["dfCO2CapZones"][z,:])) for t= 1:T)
        )
        # Sum over each policy type, each zone and each time step
        @expression(EP,eCH2EmissionsPenaltybyPolicy[cap=1:inputs["NCO2Cap"]],
        sum(inputs["omega"][t]*sum(eH2EmissionsByZone[z,t]*inputs["dfCO2Price"][z,cap] for z=findall(x->x==1, inputs["dfCO2CapZones"][:,cap])) for t= 1:T)
        )
        @expression(EP,eCH2GenTotalEmissionsPenalty,
