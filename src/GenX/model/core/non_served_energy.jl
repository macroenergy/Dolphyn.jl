...
function non_served_energy(EP::Model, inputs::Dict)
    ...
    @variable(EP, vNSE[s = 1:SEG, z = 1:Z, t = 1:T] >= 0)

    ### Expressions ###
    ## Objective Function Expressions ##
    # Cost of non-served energy/curtailed demand in zone "z" at hour "t"
    @expression(
        EP,
        eCNSE[s = 1:SEG, z = 1:Z, t = 1:T],
        inputs["omega"][t] * inputs["pC_D_Curtail"][s] * vNSE[s, z, t]
    )

    # Sum individual demand segment contributions to non-served energy costs to get total non-served energy costs
    # Julia is fastest when summing over one row one column at a time
    @expression(EP, eTotalCNSETS[z = 1:Z, t = 1:T], sum(eCNSE[s, z, t] for s = 1:SEG))
    @expression(EP, eTotalCNSET[t = 1:T], sum(eTotalCNSETS[z, t] for z = 1:Z))
    ...
    @expression(EP, ePowerBalanceNse[z = 1:Z, t = 1:T], sum(vNSE[s, z, t] for s = 1:SEG))
    ...
    @constraint(
        EP,
        cNSEPerSeg[s = 1:SEG, z = 1:Z, t = 1:T],
        vNSE[s, z, t] <= inputs["pMax_D_Curtail"][s] * inputs["pD"][z, t]
    )
    ...
    @constraint(
        EP,
        cMaxNSE[z = 1:Z, t = 1:T],
        sum(vNSE[s, z, t] for s = 1:SEG) <= inputs["pD"][z, t]
    )
