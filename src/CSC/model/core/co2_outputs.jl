...
function co2_outputs(EP::Model, inputs::Dict, setup::Dict)
	...
	# Define sets
	K = inputs["CO2_RES_ALL"] # Number of Carbon capture units
	T = inputs["T"]     # Number of time steps (hours)

	### Variables ###

    # CO2 captured from carbon capture resource k (tonnes of CO2/hr) in time t
	@variable(EP, vCO2Capture[k in 1:K, t = 1:T] >= 0)
	...
	if setup["ParameterScale"] ==1
		@expression(EP, eCCO2CaptureVar_out[k in 1:K,t = 1:T], 
	...
	@expression(EP, eTotalCCO2CaptureVarOutT[t=1:T], sum(eCCO2CaptureVar_out[k,t] for k in 1:K))
