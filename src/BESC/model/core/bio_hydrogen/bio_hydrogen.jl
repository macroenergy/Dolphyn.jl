...
function bio_hydrogen(EP::Model, inputs::Dict, setup::Dict)
	...
	@expression(EP,eBioH2_produced_mwh_per_plant_per_time[i in 1:BIO_H2_RES_ALL, t in 1:T], EP[:vBiomass_consumed_per_plant_per_time_H2][i,t] * dfBioH2[!,:Biomass_energy_MMBtu_per_tonne][i] * dfBioH2[!,:Biorefinery_efficiency][i] * dfBioH2[!,:BioH2_fraction][i] * MMBtu_to_MWh)
	@expression(EP,eBioH2_produced_mwh_per_time_per_zone[t in 1:T,z in 1:Z], sum(EP[:eBioH2_produced_mwh_per_plant_per_time][i,t] for i in intersect(1:BIO_H2_RES_ALL, dfBioH2[dfBioH2[!,:Zone].==z,:][!,:R_ID])))

	EP[:eH2Balance] += EP[:eBioH2_produced_mwh_per_time_per_zone]
