"""
    path = choose_csc_output_dir(pathinit)
...
"""
function choose_csc_output_dir(pathinit::String)
    path = joinpath(pathinit, "Results_CSC")
    counter = 1
    while isdir(path)
        path = joinpath(string(pathinit, "_", counter), "Results_CSC")
        counter += 1
    end
    return path
end
