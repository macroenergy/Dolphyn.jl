...
function h2_truck_investment(EP::Model, inputs::Dict, setup::Dict)
    ...
    @variable(EP, vH2CAPTRUCKNUMBER[j in NEW_CAP_H2_TRUCK_CHARGE] >= 0)

    # Retired charge capacity of truck type "j" from existing capacity
    @variable(EP, vH2RETCAPTRUCKNUMBER[j in RET_CAP_H2_TRUCK_CHARGE] >= 0)

	# ? Yuheng Zhang Qn: should truck compression energy be categorized into resources and stored in h2_gen
    ...
    @expression(
        EP,
        eTotalH2CapTruckNumber[j in H2_TRUCK_TYPES],
        if (j in intersect(NEW_CAP_H2_TRUCK_CHARGE, RET_CAP_H2_TRUCK_CHARGE))
            dfH2Truck[!, :Existing_Number][j] + EP[:vH2CAPTRUCKNUMBER][j] -
            EP[:vH2RETCAPTRUCKNUMBER][j]
        ...
        elseif (j in setdiff(RET_CAP_H2_TRUCK_CHARGE, NEW_CAP_H2_TRUCK_CHARGE))
            dfH2Truck[!, :Existing_Number][j] - EP[:vH2RETCAPTRUCKNUMBER][j]
        else
    ...
    @expression(
        EP,
        eTotalH2CapTruckEnergy[z = 1:Z, j in H2_TRUCK_TYPES],
        if (j in intersect(NEW_CAP_H2_TRUCK_ENERGY, RET_CAP_H2_TRUCK_ENERGY))
            dfH2Truck[!, Symbol("Existing_Energy_Cap_tonne_z$z")][j] + EP[:vH2CAPTRUCKENERGY][z, j] -
            EP[:vH2RETCAPTRUCKENERGY][z, j]
        ...
        elseif (j in setdiff(RET_CAP_H2_TRUCK_ENERGY, NEW_CAP_H2_TRUCK_ENERGY))
            dfH2Truck[!, Symbol("Existing_Energy_Cap_tonne_z$z")][j] - EP[:vH2RETCAPTRUCKENERGY][z, j]
        else
	...
	if setup["ParameterScale"] ==1
        ...
        @expression(EP, eCFixH2TruckCharge[j in H2_TRUCK_TYPES],
            if j in NEW_CAP_H2_TRUCK_CHARGE # Truck types eligible for new charge capacity
                1/ModelScalingFactor^2*(dfH2Truck[!,:H2TruckUnitCapex_per_unit][j]*vH2CAPTRUCKNUMBER[j])
            else
                EP[:vZERO]
            end
        )
        ...
        @expression(EP, eCFixH2TruckCharge[j in H2_TRUCK_TYPES],
            if j in NEW_CAP_H2_TRUCK_CHARGE # Resources eligible for new charge capacity
                dfH2Truck[!,:H2TruckUnitCapex_per_unit][j]*vH2CAPTRUCKNUMBER[j]
            else
                EP[:vZERO]
            end
        )
	...
	if setup["ParameterScale"]==1
		...
		if j in NEW_CAP_H2_TRUCK_ENERGY # Resources eligible for new capacity
			1/ModelScalingFactor^2*(dfH2Truck[!,:Inv_Cost_Energy_p_tonne_yr][j]*vH2CAPTRUCKENERGY[z, j] + dfH2Truck[!,:Fixed_OM_Cost_Energy_p_tonne_yr][j]*eTotalH2CapTruckEnergy[z, j])
		else
			1/ModelScalingFactor^2*(dfH2truck[!,:Fixed_OM_Cost_Energy_p_tonne_yr][j]*eTotalH2CapTruckEnergy[z, j])
		end
		)
	else
		@expression(EP, eCFixH2TruckEnergy[z = 1:Z, j in H2_TRUCK_TYPES],
		if j in NEW_CAP_H2_TRUCK_ENERGY # Resources eligible for new capacity
			dfH2Truck[!,:Inv_Cost_Energy_p_tonne_yr][j]*vH2CAPTRUCKENERGY[z, j] + dfH2Truck[!,:Fixed_OM_Cost_Energy_p_tonne_yr][j]*eTotalH2CapTruckEnergy[z, j]
		else
			dfH2Truck[!,:Fixed_OM_Cost_Energy_p_tonne_yr][y]*eTotalH2CapEnergy[z, j]
		end
		)
	end
	...
	#Cannot retire more charge capacity than existing charge capacity
 	@constraint(EP, cMaxRetH2TruckNumber[j in RET_CAP_H2_TRUCK_CHARGE], vH2RETCAPTRUCKNUMBER[j] <= dfH2Truck[!,:Existing_Number][j])


  	## Constraints on truck compression energy
		
	# Cannot retire more energy capacity than existing energy capacity
	@constraint(EP, cMaxRetH2TruckEnergy[z = 1:Z, j in RET_CAP_H2_TRUCK_ENERGY], vH2RETCAPTRUCKENERGY[z,j] <= dfH2Truck[!, Symbol("Existing_Energy_Cap_tonne_z$z")][j])
