...
function write_outputs(EP::Model, path::AbstractString, setup::Dict, inputs::Dict)
	...
	print_and_log("Time elapsed for writing costs is")
	print_and_log(elapsed_time_costs)
	...
	print_and_log("Time elapsed for writing storage is")
	print_and_log(elapsed_time_storage)
	dfCurtailment = write_curtailment(path, sep, inputs, setup, EP)
	elapsed_time_nse = @elapsed write_nse(path, sep, inputs, setup, EP)
	print_and_log("Time elapsed for writing nse is")
	print_and_log(elapsed_time_nse)
	elapsed_time_power_balance = @elapsed write_power_balance(path, sep, inputs, setup, EP)
	print_and_log("Time elapsed for writing power balance is")
	print_and_log(elapsed_time_power_balance)
	if inputs["Z"] > 1
		elapsed_time_flows = @elapsed write_transmission_flows(path, sep, setup, inputs, EP)
		print_and_log("Time elapsed for writing transmission flows is")
		print_and_log(elapsed_time_flows)
		elapsed_time_losses = @elapsed write_transmission_losses(path, sep, inputs, setup, EP)
		print_and_log("Time elapsed for writing transmission losses is")
		print_and_log(elapsed_time_losses)
		if setup["NetworkExpansion"] == 1
			elapsed_time_expansion = @elapsed write_nw_expansion(path, sep, inputs, setup, EP)
			print_and_log("Time elapsed for writing network expansion is")
			print_and_log(elapsed_time_expansion)
		end
	end
	elapsed_time_emissions = @elapsed write_emissions(path, sep, inputs, setup, EP)
	print_and_log("Time elapsed for writing emissions is")
	print_and_log(elapsed_time_emissions)
	if has_duals(EP) == 1
		elapsed_time_reliability = @elapsed write_reliability(path, sep, inputs, setup, EP)
		print_and_log("Time elapsed for writing reliability is")
		print_and_log(elapsed_time_reliability)
		elapsed_time_stordual = @elapsed write_storagedual(path, sep, inputs, setup, EP)
		print_and_log("Time elapsed for writing storage duals is")
		print_and_log(elapsed_time_stordual)
	end

	if setup["UCommit"] >= 1
		elapsed_time_commit = @elapsed write_commit(path, sep, inputs, setup, EP)
		print_and_log("Time elapsed for writing commitment is")
		print_and_log(elapsed_time_commit)
		elapsed_time_start = @elapsed write_start(path, sep, inputs, setup, EP)
		print_and_log("Time elapsed for writing startup is")
		print_and_log(elapsed_time_start)
		elapsed_time_shutdown = @elapsed write_shutdown(path, sep, inputs, setup, EP)
		print_and_log("Time elapsed for writing shutdown is")
		print_and_log(elapsed_time_shutdown)
		if setup["Reserves"] == 1
			elapsed_time_reg = @elapsed write_reg(path, sep, inputs, setup, EP)
			print_and_log("Time elapsed for writing regulation is")
			print_and_log(elapsed_time_reg)
			elapsed_time_rsv = @elapsed write_rsv(path, sep, inputs, setup, EP)
			print_and_log("Time elapsed for writing reserves is")
			print_and_log(elapsed_time_rsv)
		end
	end
	...
	if setup["OperationWrapping"] == 1 && !isempty(inputs["STOR_LONG_DURATION"])
		...
		print_and_log("Time elapsed for writing lds init is")
		print_and_log(elapsed_time_lds_init)
		elapsed_time_lds_dstor = @elapsed write_opwrap_lds_dstor(path, sep, inputs, setup, EP)
		print_and_log("Time elapsed for writing lds dstor is")
		print_and_log(elapsed_time_lds_dstor)
	end
	...
	print_and_log("Time elapsed for writing time weights is")
	print_and_log(elapsed_time_time_weights)
	dfESR = DataFrame()
	dfESRRev = DataFrame()
	...
	if setup["CapacityReserveMargin"]==1 && has_duals(EP) == 1
		...
		print_and_log("Time elapsed for writing reserve margin is")
		print_and_log(elapsed_time_rsv_margin)
		...
		print_and_log("Time elapsed for writing capacity value is")
		print_and_log(elapsed_time_cap_value)
	end
	...
	print_and_log("Time elapsed for writing net revenue is")
	print_and_log(elapsed_time_net_rev)
	## Print confirmation
