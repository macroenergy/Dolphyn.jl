...
\begin{aligned}
   ...
   - & \sum_{k \in {Ely}} \epsilon_{y,z,p}^{CRM} \times x_{k,z,t}^{\textrm{E,H-GEN}} + \sum_{k \in {H2G2P}} \epsilon_{y,z,p}^{CRM} \times x_{k,z,t}^{\textrm{H,H2G2P}} \\
...
function cap_reserve_margin(EP::Model, inputs::Dict, setup::Dict)
	...
	if setup["ModelH2"] == 1
		...
		if setup["ParameterScale"] ==1 # IF ParameterScale = 1, power system operation/capacity modeled in GW rather than MW 
			...
			@expression(EP, eP2G[k = H2_GEN, t = 1:T], EP[:vP2G][k,t]) 
		end
		...
		if setup["ModelH2G2P"] == 1
			...
			if setup["ParameterScale"] ==1 # IF ParameterScale = 1, power system operation/capacity modeled in GW rather than MW 
				...
				@expression(EP, ePG2P[k = 1:H2_G2P_ALL, t = 1:T], EP[:vPG2P][k,t]) 
			end
			...
			if setup["CapacityReserveMargin"] > 0
				if SEG >=2
					if Z > 1
						...
						+ sum(dfH2G2P[k,Symbol("CapRes_$res")] * EP[:ePG2P][k,t] for k in 1:H2_G2P_ALL) # including H2G2P
