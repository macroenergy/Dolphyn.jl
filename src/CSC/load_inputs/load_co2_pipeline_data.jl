...
Function for reading input parameters related to the CO2 transmission network via trunk and spur pipelines.
"""
...
function load_co2_pipeline_data(setup::Dict, path::AbstractString, sep::AbstractString, inputs_co2_nw::Dict)
    ...
    #Number of CO2 Trunk Pipelines = L
    ...
    inputs_co2_nw["CO2_Trunk_Pipe_Map"] = co2_trunk_pipe_map
    ...
    inputs_co2_nw["Trunk_pCO2_Pipe_length_miles"] = convert(Array{Float64}, collect(skipmissing(co2_trunk_pipeline_var[!,:Pipe_length_miles])))

    # Length between two booster compressor stations in miles
    inputs_co2_nw["Trunk_CO2_len_bw_comp_mile"] = convert(Array{Float64}, collect(skipmissing(co2_trunk_pipeline_var[!,:CO2_len_bw_comp_mile])))
    ...
    inputs_co2_nw["Trunk_pCO2_Pipe_Min_Cap"] = convert(Array{Float64}, collect(skipmissing(co2_trunk_pipeline_var[!,:Min_pipecap_stor_frac]))) .* inputs_co2_nw["Trunk_pCO2_Pipe_Max_Cap"]

    #Capital Cost Per Pipe using mean cost
    inputs_co2_nw["Trunk_pCAPEX_CO2_Pipe"] = convert(Array{Float64}, collect(skipmissing(co2_trunk_pipeline_var[!,:CO2Pipe_Inv_Cost_per_mile_yr]))) .* inputs_co2_nw["Trunk_pCO2_Pipe_length_miles"]
    inputs_co2_nw["Trunk_pFixed_OM_CO2_Pipe"] = convert(Array{Float64}, collect(skipmissing(co2_trunk_pipeline_var[!,:CO2Pipe_Fixed_OM_Cost_per_mile_yr]))) .* inputs_co2_nw["Trunk_pCO2_Pipe_length_miles"]
    ...
    inputs_co2_nw["Trunk_pCAPEX_Comp_CO2_Pipe"] = convert(Array{Float64}, collect(skipmissing(co2_trunk_pipeline_var[!,:BoosterCompCapex_per_tonne_p_hr_yr]))).* inputs_co2_nw["Trunk_pCO2_Pipe_Max_Flow"].*inputs_co2_nw["Trunk_CO2_no_booster_comp_stations"] 
    ...
    inputs_co2_nw["Trunk_pComp_MWh_per_tonne_CO2_Pipe"] = convert(Array{Float64}, collect(skipmissing(co2_trunk_pipeline_var[!,:BoosterCompEnergy_MWh_per_tonne]))).* inputs_co2_nw["Trunk_CO2_no_booster_comp_stations"] 
    ...
    #Create pipe number column
    co2_spur_pipe_map[!,:pipe_no] = 1:size(co2_spur_pipe_map,1)
    
    #Pivot table
    co2_spur_pipe_map = stack(co2_spur_pipe_map, 1:inputs_co2_nw["Z"]+inputs_co2_nw["S"])
    
    #Create zone column
    co2_spur_pipe_map[!,:Zone] = parse.(Float64,SubString.(co2_spur_pipe_map[!,:variable],2))
    ...
    inputs_co2_nw["CO2_Spur_Pipe_Map"] = co2_spur_pipe_map
    ...
    # Length in miles of each pipeline
    inputs_co2_nw["Spur_pCO2_Pipe_length_miles"] = convert(Array{Float64}, collect(skipmissing(co2_spur_pipeline_var[!,:Pipe_length_miles])))
    ...
    #Maximum Pipeline storage capacity in tonnes per pipe
    inputs_co2_nw["Spur_pCO2_Pipe_Max_Cap"] = convert(Array{Float64}, collect(skipmissing(co2_spur_pipeline_var[!,:CO2PipeCap_tonne_per_mile]))) .* inputs_co2_nw["Spur_pCO2_Pipe_length_miles"]
    ...
    #Capital Cost Per Pipe using mean cost
    inputs_co2_nw["Spur_pCAPEX_CO2_Pipe"] = convert(Array{Float64}, collect(skipmissing(co2_spur_pipeline_var[!,:CO2Pipe_Inv_Cost_per_mile_yr]))) .* inputs_co2_nw["Spur_pCO2_Pipe_length_miles"]
    inputs_co2_nw["Spur_pFixed_OM_CO2_Pipe"] = convert(Array{Float64}, collect(skipmissing(co2_spur_pipeline_var[!,:CO2Pipe_Fixed_OM_Cost_per_mile_yr]))) .* inputs_co2_nw["Spur_pCO2_Pipe_length_miles"]
    inputs_co2_nw["Spur_pMWh_per_tonne_CO2_Pipe"] = convert(Array{Float64}, collect(skipmissing(co2_spur_pipeline_var[!,:CO2Pipe_Energy_MWh_per_mile_per_tonne]))) .* inputs_co2_nw["Spur_pCO2_Pipe_length_miles"]

    inputs_co2_nw["Spur_pLoss_tonne_per_tonne_CO2_Pipe"] = convert(Array{Float64}, collect(skipmissing(co2_spur_pipeline_var[!,:CO2PipeLoss_tonne_per_mile_per_tonne]))) .* inputs_co2_nw["Spur_pCO2_Pipe_length_miles"]
    
    #Capital cost associated with booster compressors per pipe= capex per tonne/hour flow rate x pipe max flow rate (tonne/hour) x number of booster compressor stations per pipe route
    inputs_co2_nw["Spur_pCAPEX_Comp_CO2_Pipe"] = convert(Array{Float64}, collect(skipmissing(co2_spur_pipeline_var[!,:BoosterCompCapex_per_tonne_p_hr_yr]))).* inputs_co2_nw["Spur_pCO2_Pipe_Max_Flow"].*inputs_co2_nw["Spur_CO2_no_booster_comp_stations"] 
    ...
    println("CO2_spur_pipelines.csv Successfully Read!")
