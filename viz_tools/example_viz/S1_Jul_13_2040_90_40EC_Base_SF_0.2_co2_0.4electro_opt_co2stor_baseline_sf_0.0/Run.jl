...
syn_fuel_settings = joinpath(settings_path, "syn_fuel_settings.yml") #Settings YAML file path for CSC model
...
mysetup_syn_fuels = YAML.load(open(syn_fuel_settings))
...
mysetup = Dict()
mysetup = merge( mysetup_csc, mysetup_hsc, mysetup_genx, mysetup_syn_fuels, mysetup_global) #Merge dictionary - value of common keys will be overwritten by value in global_model_settings
...
# ### Load Syn Fuel inputs if modeling synfuels
...
set_optimizer_attribute(EP, "LogFile", "my_log_file.txt")
...
if mysetup["ModelSynFuels"] == 1
    outpath_synfuels = "$outpath/Results_SynFuels"
    write_synfuel_outputs(EP, outpath_synfuels, mysetup, myinputs)
end
