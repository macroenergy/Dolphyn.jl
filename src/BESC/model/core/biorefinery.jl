...
function biorefinery(EP::Model, inputs::Dict, setup::Dict)
	...
	#add_to_expression!(EP[:ePowerBalance],-EP[:eBioPower_consumption_per_time_per_zone])
	...
	#add_to_expression!(EP[:eBIONetpowerConsumptionByAll],EP[:eBioPower_consumption_per_time_per_zone])
	...
	#Add to power balance to take power away from generated
	#add_to_expression!(EP[:eH2Balance], -EP[:eScaled_BioH2_consumption_per_time_per_zone])
	...
	if setup["BIO_Electricity_On"] == 1
		...
		#add_to_expression!(EP[:ePowerBalance], EP[:eBioelectricity_produced_MWh_per_time_per_zone])
	...
	if setup["BIO_H2_On"] == 1
		...
		#add_to_expression!(EP[:eH2Balance], EP[:eScaled_BioH2_produced_tonne_per_time_per_zone])
	...
	if setup["BIO_Gasoline_On"] == 1
		...
		add_to_expression!(EP[:eLFGasolineBalance], EP[:eBiogasoline_produced_MMBtu_per_time_per_zone])
	...
	if setup["BIO_Diesel_On"] == 1
		...
		add_to_expression!(EP[:eLFDieselBalance], EP[:eBiodiesel_produced_MMBtu_per_time_per_zone])
	...
	if setup["BIO_Ethanol_On"] == 1
		...
		add_to_expression!(EP[:eEthanolBalance], EP[:eBioethanol_produced_MMBtu_per_time_per_zone])
