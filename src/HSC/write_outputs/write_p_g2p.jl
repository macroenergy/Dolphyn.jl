...
function write_p_g2p(path::AbstractString, sep::AbstractString, inputs::Dict, setup::Dict, EP::Model)
    ...
    dfPG2POut.AnnualSum .= pg2p * inputs["omega"] 

    # Load hourly values
    dfPG2POut = hcat(dfPG2POut, DataFrame(pg2p, :auto))
    ...
    dfPower = vcat(dfPG2POut, total)
     ...
     CSV.write(joinpath(path, "HSC_G2P_power_generation.csv"), dftranspose(dfPG2POut, false), writeheader=false)
