...
@doc raw"""
	bio_hydrogen_var_cost(EP::Model, inputs::Dict, setup::Dict)
...
The variables defined in this file named after ```vBiomass_consumed_per_plant_per_time_H2``` covers all variables $x_{r,t}^{\textrm{B,Bio}}$.

The variables defined in this file named after ```vPower_BIO_H2``` covers all variables $x_{r,t}^{\textrm{E,Bio}}$ (if any).
...
```
"""
function bio_hydrogen_var_cost(EP::Model, inputs::Dict, setup::Dict)

	println(" -- Bio Hydrogen Variable Cost Module")
	...
	#####################################################################################################################################
	#Variables
	@variable(EP,vBiomass_consumed_per_plant_per_time_H2[i = 1:BIO_H2_RES_ALL, t = 1:T] >= 0)

	#####################################################################################################################################
	#Variable cost per plant per time
	@expression(EP, eVar_Cost_BIO_H2_per_plant_per_time[i = 1:BIO_H2_RES_ALL, t = 1:T], inputs["omega"][t] * EP[:vBiomass_consumed_per_plant_per_time_H2][i,t] * dfBioH2[!,:Var_OM_per_tonne][i])

	#Variable cost per plant
	@expression(EP, eVar_Cost_BIO_H2_per_plant[i = 1:BIO_H2_RES_ALL], sum(EP[:eVar_Cost_BIO_H2_per_plant_per_time][i,t] for t in 1:T))

	#Total variable cost
	@expression(EP, eVar_Cost_BIO_H2, sum(EP[:eVar_Cost_BIO_H2_per_plant][i] for i in 1:BIO_H2_RES_ALL))

	EP[:eObj] += EP[:eVar_Cost_BIO_H2]
