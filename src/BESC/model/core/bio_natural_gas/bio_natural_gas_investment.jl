...
@doc raw"""
	bio_natural_gas_investment(EP::Model, inputs::Dict, setup::Dict)
...
The expression defined in this file named after ```vCapacity_BIO_NG_per_type``` covers all variables $y_{r}^{\textrm{B,Bio}}$.
...
```
"""
function bio_natural_gas_investment(EP::Model, inputs::Dict, setup::Dict)
	
	println(" -- Bio Natural Gas Fixed Cost Module")
	...
	@variable(EP,vCapacity_BIO_NG_per_type[i = 1:BIO_NG_RES_ALL])

	BIO_NG_Capacity_Min_Limit = dfBioNG[!,:Min_capacity_tonne_per_hr]
	BIO_NG_Capacity_Max_Limit = dfBioNG[!,:Max_capacity_tonne_per_hr]
	
	BIO_NG_Inv_Cost_per_tonne_per_hr_yr = dfBioNG[!,:Inv_Cost_per_tonne_per_hr_yr]
	BIO_NG_Fixed_OM_Cost_per_tonne_per_hr_yr = dfBioNG[!,:Fixed_OM_Cost_per_tonne_per_hr_yr]
	...
	#Fixed OM cost
	@expression(EP, eFixed_OM_BIO_NG_per_type[i = 1:BIO_NG_RES_ALL], EP[:vCapacity_BIO_NG_per_type][i] * BIO_NG_Fixed_OM_Cost_per_tonne_per_hr_yr[i])

	#Total fixed cost = CAPEX + Fixed OM
	@expression(EP, eFixed_Cost_BIO_NG_per_type[i = 1:BIO_NG_RES_ALL], EP[:eFixed_OM_BIO_NG_per_type][i] + EP[:eCAPEX_BIO_NG_per_type][i])

	#Total cost
	#Expression for total CAPEX for all resoruce types (For output)
	@expression(EP,eCAPEX_BIO_NG_total, sum(EP[:eCAPEX_BIO_NG_per_type][i] for i in 1:BIO_NG_RES_ALL))

	#Expression for total Fixed OM for all resoruce types (For output)
	@expression(EP,eFixed_OM_BIO_NG_total, sum(EP[:eFixed_OM_BIO_NG_per_type][i] for i in 1:BIO_NG_RES_ALL))

	#Expression for total Fixed Cost for all resoruce types (For output and to add to objective function)
	@expression(EP,eFixed_Cost_BIO_NG_total, sum(EP[:eFixed_Cost_BIO_NG_per_type][i] for i in 1:BIO_NG_RES_ALL))

	EP[:eObj] += EP[:eFixed_Cost_BIO_NG_total]
