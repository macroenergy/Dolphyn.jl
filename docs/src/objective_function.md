...
In the bioenergy supply chain, cost terms include bioenergy resources investment and operational costs, as well as biomass resource costs.

```math
\begin{equation*}
	Obj_{Bio} = \textrm{C}^{\textrm{Bio,c}} + \textrm{C}^{\textrm{Bio,o}} + \textrm{C}^{\textrm{Herb,o}} + \textrm{C}^{\textrm{Wood,o}}
\end{equation*}
```
...
The first term $\textrm{C}^{\textrm{Bio,c}}$ represents the fixed costs of bioenergy resources over all zones and technologies (e.g. $r \in \mathcal{R}$), which reflects the sum of the annualized capital cost, $\textrm{c}_{r}^{\textrm{Bio,INV}}$, times the total capacity $y_{r, z}^{\textrm{B,Bio}}$, plus the Fixed O&M cost, $\textrm{c}_{r}^{\textrm{Bio,FOM}}$, times the total capacity $y_{r, z}^{\textrm{B,Bio}}$.

The second term $\textrm{C}^{\textrm{Bio,o}}$ correspond to the operational cost across all zones, technologies, and time steps. It represents the sum of variable O&M cost, $\textrm{c}_{r}^{Bio,VOM}$ times the biomass input to bioenergy resource in time step $t$, $x_{r,t}^{\textrm{B,Bio}}$, and the weight of each time step $t$, $\omega_t$. 

The third term $\textrm{C}^{\textrm{Herb,o}}$ correspond to the biomass resource cost across all zones, technologies, and time steps. It represents the sum of herbaceous biomass cost, $\textrm{c}_{r}^{Herb,VOM}$ times the biomass input to herbaceous bioenergy resource in time step $t$, $x_{r,t}^{\textrm{B,Herb}}$, and the weight of each time step $t$, $\omega_t$. 

The fourth term $\textrm{C}^{\textrm{Wood,o}}$ correspond to the biomass resource cost across all zones, technologies, and time steps. It represents the sum of wood biomass cost, $\textrm{c}_{r}^{Wood,VOM}$ times the biomass input to wood bioenergy resource in time step $t$, $x_{r,t}^{\textrm{B,Wood}}$, and the weight of each time step $t$, $\omega_t$.
