...
function h2_production_commit(EP::Model, inputs::Dict, setup::Dict)
    ...
    if setup["ParameterScale"] == 1
        @expression(
            ...
            (
                inputs["omega"][t] * inputs["C_H2_Start"][k] * vH2GenStart[k, t] /
    ...
    @expression(
        EP,
        eTotalH2GenCStartT[t = 1:T],
        sum(eH2GenCStart[k, t] for k in H2_GEN_COMMIT)
    )
    ...
    @expression(
        EP,
        eH2GenCommit[t = 1:T, z = 1:Z],
        sum(
            EP[:vH2Gen][k, t] for
            k in intersect(H2_GEN_COMMIT, dfH2Gen[dfH2Gen[!, :Zone].==z, :][!, :R_ID])
        )
    )
    ...
    if setup["ParameterScale"] == 1 # IF ParameterScale = 1, power system operation/capacity modeled in GW rather than MW
        @expression(
            EP,
            ePowerBalanceH2GenCommit[t = 1:T, z = 1:Z],
            sum(
                EP[:vP2G][k, t] / ModelScalingFactor for
                k in intersect(H2_GEN_COMMIT, dfH2Gen[dfH2Gen[!, :Zone].==z, :][!, :R_ID])
            )
        )
    ...
    @constraints(
        ...
        begin
            [k in H2_GEN_COMMIT, t = 1:T],
            EP[:vH2GenCOMMIT][k, t] <=
            EP[:eH2GenTotalCap][k] / dfH2Gen[!, :Cap_Size_tonne_p_hr][k]
            [k in H2_GEN_COMMIT, t = 1:T],
            EP[:vH2GenStart][k, t] <=
            EP[:eH2GenTotalCap][k] / dfH2Gen[!, :Cap_Size_tonne_p_hr][k]
            [k in H2_GEN_COMMIT, t = 1:T],
    ...
    @constraints(
        ...
        begin
            ...
            [k in H2_GEN_COMMIT, t in START_SUBPERIODS],
            EP[:vH2GenCOMMIT][k, t] ==
            EP[:vH2GenCOMMIT][k, (t+hours_per_subperiod-1)] + EP[:vH2GenStart][k, t] -
            EP[:vH2GenShut][k, t]
            ...
            [k in H2_GEN_COMMIT, t in INTERIOR_SUBPERIODS],
            EP[:vH2GenCOMMIT][k, t] ==
            EP[:vH2GenCOMMIT][k, t-1] + EP[:vH2GenStart][k, t] - EP[:vH2GenShut][k, t]
        end
    )
    ...
    @constraint(
        EP,
        [k in H2_GEN_COMMIT, t in START_SUBPERIODS],
        EP[:vH2Gen][k, t] - EP[:vH2Gen][k, (t+hours_per_subperiod-1)] <=
        dfH2Gen[!, :Ramp_Up_Percentage][k] *
        dfH2Gen[!, :Cap_Size_tonne_p_hr][k] *
        (EP[:vH2GenCOMMIT][k, t] - EP[:vH2GenStart][k, t]) +
        min(
            inputs["pH2_Max"][k, t],
            max(dfH2Gen[!, :H2Gen_min_output][k], dfH2Gen[!, :Ramp_Up_Percentage][k]),
        ) *
        dfH2Gen[!, :Cap_Size_tonne_p_hr][k] *
        EP[:vH2GenStart][k, t] -
        dfH2Gen[!, :H2Gen_min_output][k] *
        dfH2Gen[!, :Cap_Size_tonne_p_hr][k] *
        EP[:vH2GenShut][k, t]
    )
    ...
    @constraint(
        EP,
        [k in H2_GEN_COMMIT, t in START_SUBPERIODS],
        EP[:vH2Gen][k, (t+hours_per_subperiod-1)] - EP[:vH2Gen][k, t] <=
        dfH2Gen[!, :Ramp_Down_Percentage][k] *
        dfH2Gen[!, :Cap_Size_tonne_p_hr][k] *
        (EP[:vH2GenCOMMIT][k, t] - EP[:vH2GenStart][k, t]) -
        dfH2Gen[!, :H2Gen_min_output][k] *
        dfH2Gen[!, :Cap_Size_tonne_p_hr][k] *
        ...
        min(
            inputs["pH2_Max"][k, t],
            max(dfH2Gen[!, :H2Gen_min_output][k], dfH2Gen[!, :Ramp_Down_Percentage][k]),
        ) *
        dfH2Gen[!, :Cap_Size_tonne_p_hr][k] *
        EP[:vH2GenShut][k, t]
    )
    ...
    for y in H2_GEN_COMMIT
        ...
        @constraints(
            ...
            begin
                ...
                EP[:vH2GenCOMMIT][y, t] >=
                sum(EP[:vH2GenStart][y, e] for e = (t-dfH2Gen[!, :Up_Time][y]):t)
                ...
                [t in Up_Time_HOURS],
                EP[:vH2GenCOMMIT][y, t] >=
                sum(EP[:vH2GenStart][y, e] for e in (t-((t%hours_per_subperiod)-1):t)) +
                sum(
                    EP[:vH2GenStart][y, e] for e =
                        ((t+hours_per_subperiod-(t%hours_per_subperiod))-(dfH2Gen[
                            !,
                            :Up_Time,
                ...
                EP[:vH2GenCOMMIT][y, t] >=
                EP[:vH2GenStart][y, t] + sum(
                    EP[:vH2GenStart][y, e] for e =
                        ((t+hours_per_subperiod-1)-(dfH2Gen[!, :Up_Time][y]-1)):(t+hours_per_subperiod-1)
                )
            end
        )
        ...
        @constraints(
            ...
            begin
                ...
                EP[:eH2GenTotalCap][y] / dfH2Gen[!, :Cap_Size_tonne_p_hr][y] -
                EP[:vH2GenCOMMIT][y, t] >=
                sum(EP[:vH2GenShut][y, e] for e = (t-dfH2Gen[!, :Down_Time][y]):t)
                ...
                EP[:eH2GenTotalCap][y] / dfH2Gen[!, :Cap_Size_tonne_p_hr][y] -
                EP[:vH2GenCOMMIT][y, t] >=
                sum(EP[:vH2GenShut][y, e] for e in (t-((t%hours_per_subperiod)-1):t)) +
                sum(
                    EP[:vH2GenShut][y, e] for e =
                        ((t+hours_per_subperiod-(t%hours_per_subperiod))-(dfH2Gen[
                            !,
                            :Down_Time,
                        ][y]-(t%hours_per_subperiod))):(t+hours_per_subperiod-(t%hours_per_subperiod))
                ...
                EP[:eH2GenTotalCap][y] / dfH2Gen[!, :Cap_Size_tonne_p_hr][y] -
                EP[:vH2GenCOMMIT][y, t] >=
                EP[:vH2GenShut][y, t] + sum(
                    EP[:vH2GenShut][y, e] for e =
                        ((t+hours_per_subperiod-1)-(dfH2Gen[!, :Down_Time][y]-1)):(t+hours_per_subperiod-1)
                )
            end
        )
    end
