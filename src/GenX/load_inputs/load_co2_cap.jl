...
function load_co2_cap(setup::Dict, path::AbstractString, inputs_co2::Dict)
	...
	cap = count(s -> startswith(String(s), "CO2_Cap"), names(inputs_co2["dfCO2Cap"]))
	first_col = findall(s -> s == "CO2_Cap_1", names(inputs_co2["dfCO2Cap"]))[1]
	last_col = findall(s -> s == "CO2_Cap_$cap", names(inputs_co2["dfCO2Cap"]))[1]
