...
|Start\_Cost\_per\_MW\_p\_hr |Cost per MWh of nameplate capacity to start a generator (\$/MWh per start). Multiplied by the number of generation units (each with a pre-specified nameplate capacity) that is turned on.|
