...
function co2_cap!(EP::Model, inputs::Dict, setup::Dict)
	...
	@variable(EP, vCO2Emissions_external_GenX[z=1:Z,t=1:T] >=0)
	...
	if setup["CO2Cap"] == 1
		@constraint(EP, cCO2Emissions_systemwide[cap=1:inputs["NCO2Cap"]],
			+ sum(inputs["omega"][t] * (vCO2Emissions_external_GenX[z,t] + EP[:eEmissionsByZone][z,t]) for z=findall(x->x==1, inputs["dfCO2CapZones"][:,cap]), t=1:T) 
			- vCO2Cap_slack[cap]
			<=
