...
@doc raw"""
	write_bio_zone_biodiesel_produced(path::AbstractString, sep::AbstractString, inputs::Dict, setup::Dict, EP::Model)

Function for reporting the biodiesel produced across different zones with time.
"""
	...
	for z in 1:Z
	   	...
	   	for t in 1:T
			if setup["ParameterScale"]==1
	     		dfTemp1[t+rowoffset,1]= sum(value.(EP[:eBiodiesel_produced_per_plant_per_time][dfbioenergy[(dfbioenergy[!,:Zone].==z),:][!,:R_ID],t]))*ModelScalingFactor
			else
				dfTemp1[t+rowoffset,1]= sum(value.(EP[:eBiodiesel_produced_per_plant_per_time][dfbioenergy[(dfbioenergy[!,:Zone].==z),:][!,:R_ID],t]))
			end
	   	end
