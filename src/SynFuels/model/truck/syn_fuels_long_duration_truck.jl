...
"""
function syn_fuels_long_duration_truck(EP::Model, inputs::Dict)

    println("Synthesis Fuels Long Duration Truck Module")
    ...
    @variable(
        EP,
        0 >= vSynTruckSOCw[z = 1:Z, j in SYN_TRUCK_TYPES, n in MODELED_HOURS_INDEX] >= 0
    )

    # Build up in truck inventory over each representative period w
    # Build up inventory is fixed zero
    @variable(EP, 0 >= vSynTruckdSOC[z = 1:Z, j in SYN_TRUCK_TYPES, w = 1:REP_PERIOD] >= 0)

    @constraints(
        EP,
        begin
            # State of charge of truck at beginning of each modeled period cannot exceed installed energy capacity
            [z = 1:Z, j in SYN_TRUCK_TYPES, n in MODELED_HOURS_INDEX],
            vSynTruckSOCw[z, j, n] <= vSynNTruck[j]

            # State of charge of truck balance
            [z = 1:Z, j in SYN_TRUCK_TYPES, n = N],
                vSynTruckSOCw[z, j, 1] ==
                vSynTruckSOCw[z, j, n] +
                vSynTruckdSOC[z, j, dfPeriodMap[!, :RepPeriod_index][n]]
            [z = 1:Z, j in SYN_TRUCK_TYPES, n = 1:N-1],
                vSynTruckSOCw[z, j, n+1] ==
                vSynTruckSOCw[z, j, n] +
                vSynTruckdSOC[z, j, dfPeriodMap[!, :RepPeriod_index][n]]
        end
    )
