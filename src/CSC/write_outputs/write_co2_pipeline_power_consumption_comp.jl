...
function write_co2_pipeline_power_consumption_comp(path::AbstractString, sep::AbstractString, inputs::Dict, setup::Dict, EP::Model)
	...
	for z in 1:Z
	   	...
	   	dfTemp1[1,1:size(dfTemp1,2)] = ["CO2 Pipeline Compressors Power Consumption"]
	   	dfTemp1[2,1:size(dfTemp1,2)] = repeat([z],size(dfTemp1,2))

	   	for t in 1:T
			if setup["ParameterScale"]==1
	     		dfTemp1[t+rowoffset,1]= value(EP[:ePowerDemandCO2PipeCompression_zt][z,t])*ModelScalingFactor
			else
				dfTemp1[t+rowoffset,1]= value(EP[:ePowerDemandCO2PipeCompression_zt][z,t])
			end
	   	end
	...
	CSV.write(string(path,sep,"CSC_co2_pipeline_power_consumption_zone_comp.csv"), dfCO2PipePowerConsumption, writeheader=false)
end
