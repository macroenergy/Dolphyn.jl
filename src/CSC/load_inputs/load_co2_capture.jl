function load_co2_capture(setup::Dict, path::AbstractString, sep::AbstractString, inputs_capture::Dict)
	...
	# Set of CO2 storage resources
	#inputs_capture["CO2_STOR_ALL"] = co2_capture[co2_capture.CO2_STOR.==1,:R_ID]

	# Defining whether CO2 storage is modeled as long-duration (inter-period energy transfer allowed) or short-duration storage (inter-period energy transfer disallowed)
	#inputs_capture["CO2_STOR_LONG_DURATION"] = co2_capture[(co2_capture.LDS.==1) .& (co2_capture.CO2_STOR.==1),:R_ID]
	#inputs_capture["CO2_STOR_SHORT_DURATION"] = co2_capture[(co2_capture.LDS.==0) .& (co2_capture.CO2_STOR.==1),:R_ID]

	# Set of all storage resources eligible for new energy capacity
	#inputs_capture["NEW_CAP_CO2_ENERGY"] = intersect(co2_capture[co2_capture.New_Build.==1,:R_ID], co2_capture[co2_capture.Max_Energy_Cap_tonne.!=0,:R_ID], inputs_capture["CO2_STOR_ALL"])
	# Set of all storage resources eligible for energy capacity retirements
	#inputs_capture["RET_CAP_CO2_ENERGY"] = intersect(co2_capture[co2_capture.New_Build.!=-1,:R_ID], co2_capture[co2_capture.Existing_Energy_Cap_tonne.>0,:R_ID], inputs_capture["CO2_STOR_ALL"])
	...
	#inputs_capture["NEW_CAP_CO2_CHARGE"] = intersect(co2_capture[co2_capture.New_Build.==1,:R_ID], co2_capture[co2_capture.Max_Charge_Cap_tonne_p_hr.!=0,:R_ID], inputs_capture["CO2_STOR_ALL"])
	# Set of asymmetric charge/discharge storage resources eligible for charge capacity retirements
	#inputs_capture["RET_CAP_CO2_CHARGE"] = intersect(co2_capture[co2_capture.New_Build.!=-1,:R_ID], co2_capture[co2_capture.Existing_Charge_Cap_tonne_p_hr.>0,:R_ID], inputs_capture["CO2_STOR_ALL"])
