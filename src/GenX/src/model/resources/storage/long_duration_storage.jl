...
function long_duration_storage!(EP::Model, inputs::Dict)
	...
	@constraint(EP, cSoCLongDurationStorageMaxInt[y in STOR_LONG_DURATION, r in MODELED_PERIODS_INDEX],
					(1-dfGen[y,:Self_Disch])*vSOCw[y,r]-(1/dfGen[y,:Eff_Down]*EP[:vP][y,hours_per_subperiod*(dfPeriodMap[r,:Rep_Period_Index]-1)+1])
					+(dfGen[y,:Eff_Up]*EP[:vCHARGE][y,hours_per_subperiod*(dfPeriodMap[r,:Rep_Period_Index]-1)+1])
