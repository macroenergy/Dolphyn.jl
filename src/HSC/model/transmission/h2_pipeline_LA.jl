...
function h2_pipeline(EP::Model, inputs::Dict, setup::Dict)
    ...
    if setup["H2PipeDirection"] == 1
        @constraint(EP, cSetPositiveFlowtoZero[p = 1:H2_P, t=1:T, d = 1], vH2PipeFlow_pos[p, t, d] .== 0)
        @constraint(EP, cSetNegativeFlowtoZero[p = 1:H2_P, t=1:T, d = -1], vH2PipeFlow_neg[p, t, d] .== 0)
    end
