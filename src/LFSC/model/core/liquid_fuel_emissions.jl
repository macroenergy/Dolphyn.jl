...
function liquid_fuel_emissions(EP::Model, inputs::Dict, setup::Dict)
    ...
    if setup["ModelSyntheticFuels"] == 1
        ...
        #######CO2 capture from synthetic fuels plant
        ...
        ##########CO2 emitted as a result of syn fuel consumption in each plant
        #CO2 emitted as a result of syn diesel consumption
        @expression(EP,eSyn_Diesel_CO2_Emissions_By_Plant[k = 1:SYN_FUELS_RES_ALL, t=1:T], 
        Syn_diesel_co2_per_mmbtu * EP[:eSFProd_Diesel_Plant][k,t])

        #CO2 emitted as a result of syn jetfuel consumption
        @expression(EP,eSyn_Jetfuel_CO2_Emissions_By_Plant[k = 1:SYN_FUELS_RES_ALL, t=1:T], 
        Syn_jetfuel_co2_per_mmbtu * EP[:eSFProd_Jetfuel_Plant][k,t])

        #CO2 emitted as a result of syn gasoline consumption
        @expression(EP,eSyn_Gasoline_CO2_Emissions_By_Plant[k = 1:SYN_FUELS_RES_ALL, t=1:T], 
        Syn_gasoline_co2_per_mmbtu * EP[:eSFProd_Gasoline_Plant][k,t])

        ###########################################################################

        ##########CO2 emitted as a result of syn fuel consumption in each zone
        #CO2 emitted as a result of syn diesel consumption
        @expression(EP,eSyn_Diesel_CO2_Emissions_By_Zone[z = 1:Z,t=1:T], 
        Syn_diesel_co2_per_mmbtu * EP[:eSynFuelProd_Diesel_Zone][t,z])

        #CO2 emitted as a result of syn jetfuel consumption
        @expression(EP,eSyn_Jetfuel_CO2_Emissions_By_Zone[z = 1:Z,t=1:T], 
        Syn_jetfuel_co2_per_mmbtu * EP[:eSynFuelProd_Jetfuel_Zone][t,z])

        #CO2 emitted as a result of syn gasoline consumption
        @expression(EP,eSyn_Gasoline_CO2_Emissions_By_Zone[z = 1:Z,t=1:T], 
        Syn_gasoline_co2_per_mmbtu * EP[:eSynFuelProd_Gasoline_Zone][t,z])

        ############################################################################

        ########CO2 emitted per type of resource "k" #No need to scale ratio
        @expression(EP,eSyn_Fuels_CO2_Vent_Emissions_By_Res[k=1:SYN_FUELS_RES_ALL,t=1:T], (EP[:eSyn_Diesel_CO2_Emissions_By_Plant][k,t] + EP[:eSyn_Jetfuel_CO2_Emissions_By_Plant][k,t] + EP[:eSyn_Gasoline_CO2_Emissions_By_Plant][k,t]))
        

        #CO2 emitted by fuel usage per zone
        @expression(EP, eSynfuels_Production_CO2_Emissions_By_Zone[z=1:Z, t=1:T], 
            sum(eSyn_Fuels_CO2_Vent_Emissions_By_Res[k,t] for k in dfSynFuels[(dfSynFuels[!,:Zone].==z),:R_ID]))
