...
@doc raw"""
    load_syn_fuels_pipeline(path::AbstractString, setup::Dict, inputs::Dict)

Function for reading input parameters related to the synthesis fuels transmission network.
"""
function load_syn_fuels_pipeline_data(path::AbstractString, setup::Dict, inputs::Dict)

    # Network zones inputs and Network topology inputs
    pipeline_var = DataFrame(
        CSV.File(joinpath(path, "Syn_pipelines.csv"), header = true),
        copycols = true,
    )
    ...
    pipeline_var = filter(
        row -> (
            row.StartZone in ["z$z" for z in Zones] && row.EndZone in ["z$z" for z in Zones]
        ),
        pipeline_var,
    )
    ...
    println("Syn_pipelines.csv Successfully Read!")
