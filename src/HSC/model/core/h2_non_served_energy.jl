...
function h2_non_served_energy(EP::Model, inputs::Dict, setup::Dict)
    ...
    @variable(EP, vH2NSE[s = 1:H2_SEG, z = 1:Z, t = 1:T] >= 0)
    ...
    @expression(
        EP,
        eH2CNSE[s = 1:H2_SEG, z = 1:Z, t = 1:T],
        (inputs["omega"][t] * inputs["pC_H2_D_Curtail"][s] * vH2NSE[s, z, t])
    )
    ...
    @expression(
        EP,
        eTotalH2CNSETS[z = 1:Z, t = 1:T],
        sum(eH2CNSE[s, z, t] for s = 1:H2_SEG)
    )
    @expression(EP, eTotalH2CNSET[t = 1:T], sum(eTotalH2CNSETS[z, t] for z = 1:Z))
    ...
    @expression(EP, eH2BalanceNse[z = 1:Z, t = 1:T], sum(vH2NSE[s, z, t] for s = 1:H2_SEG))
    ...
    @constraint(
        EP,
        cH2NSEPerSeg[s = 1:H2_SEG, z = 1:Z, t = 1:T],
        vH2NSE[s, z, t] <= inputs["pMax_H2_D_Curtail"][s] * inputs["H2_D"][z, t]
    )

    # Total demand curtailed in each time step (hourly) cannot exceed total demand
    @constraint(
        EP,
        cMaxH2NSE[z = 1:Z, t = 1:T],
        sum(vH2NSE[s, z, t] for s = 1:H2_SEG) <= inputs["H2_D"][z, t]
    )
