...
function write_bio_hydrogen_plant_capacity(path::AbstractString, sep::AbstractString, inputs::Dict, setup::Dict, EP::Model)
	...
	for i in 1:inputs["BIO_H2_RES_ALL"]
		...
		if value(EP[:vCapacity_BIO_H2_per_type][i]) > 0.01

			capbioenergy_H2[i] = value(EP[:vCapacity_BIO_H2_per_type][i])
			capbiohydrogen[i] = value(EP[:vCapacity_BIO_H2_per_type][i]) * dfBioH2[!,:Biomass_energy_MMBtu_per_tonne][i] * dfBioH2[!,:Biorefinery_efficiency][i] * dfBioH2[!,:BioH2_fraction][i] * MMBtu_to_MWh
			...
			AnnualBioHydrogen[i] = sum(inputs["omega"].* (value.(EP[:eBioH2_produced_mwh_per_plant_per_time])[i,:]))
