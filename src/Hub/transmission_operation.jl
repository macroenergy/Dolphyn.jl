...
### This file keeps the operation variables for Benders Decomposition
function transmission_operation!(EP::Model,inputs::Dict,setup::Dict)

	println("Transmission Operation Module")
    ...
    @expression(EP, eNet_Export_Flows[z=1:Z,t=1:T], sum(inputs["pNet_Map"][l,z] * EP[:vFLOW][l,t] for l=1:L))
	...
	@expression(EP, ePowerBalanceLossesByZone[t=1:T, z=1:Z],
     ...
     # change electricity balance among zones from GenX to Hub
     EP[:eElec_Hub] += ePowerBalanceLossesByZone
     EP[:eElec_Hub] += ePowerBalanceNetExportFlows
	...
	@constraints(EP, begin
		cMaxFlow_out[l=1:L, t=1:T], EP[:vFLOW][l,t] <= EP[:eAvail_Trans_Cap][l]
		cMaxFlow_in[l=1:L, t=1:T], EP[:vFLOW][l,t] >= -EP[:eAvail_Trans_Cap][l]
	end)
	...
	if TRANS_LOSS_SEGS == 1
		...
		@constraints(EP, begin
			...
			cTAuxSum[l in LOSS_LINES, t=1:T], vTAUX_POS[l,t]-vTAUX_NEG[l,t] == EP[:vFLOW][l,t]
	...
	if (TRANS_LOSS_SEGS > 1)
		...
		@constraints(EP, begin
			cTAuxSumPos[l in LOSS_LINES, t=1:T], sum(vTAUX_POS[l,s,t] for s=1:TRANS_LOSS_SEGS)-vTAUX_POS[l,0,t]  == EP[:vFLOW][l,t]
			cTAuxSumNeg[l in LOSS_LINES, t=1:T], sum(vTAUX_NEG[l,s,t] for s=1:TRANS_LOSS_SEGS) - vTAUX_NEG[l,0,t]  == -EP[:vFLOW][l,t]
		end)
