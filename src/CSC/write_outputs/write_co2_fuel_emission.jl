...
function write_co2_fuel_emission(path::AbstractString, sep::AbstractString, inputs::Dict, setup::Dict, EP::Model)
	...
	CO2_RESOURCES_NAME = inputs["CO2_RESOURCES_NAME"]
	
	if !isempty(CO2_RESOURCES_NAME)
		...
		for z in 1:Z
			...
			for t in 1:T
				if setup["ParameterScale"]==1
					dfTemp1[t+rowoffset,1]= sum(value.(EP[:eDAC_Fuel_CO2_Production_per_type][dfCO2Capture[(dfCO2Capture[!,:CO2_CAPTURE_TYPE].>0) .&  (dfCO2Capture[!,:Zone].==z),:][!,:R_ID],t]))*ModelScalingFactor
				else
