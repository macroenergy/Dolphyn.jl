	   	...
	   	for t in 1:T
			...
			if setup["ModelBIO"] == 1
				if setup["ParameterScale"]==1
					dfTemp1[t+rowoffset,2] = sum(value.(EP[:eBiohydrogen_produced_per_plant_per_time][dfbiorefinery[(dfbiorefinery[!,:Zone].==z),:][!,:R_ID],t])*ModelScalingFactor) 
				else
					dfTemp1[t+rowoffset,2] = sum(value.(EP[:eBiohydrogen_produced_per_plant_per_time][dfbiorefinery[(dfbiorefinery[!,:Zone].==z),:][!,:R_ID],t])) 
				end
			end

	     	if !isempty(intersect(dfH2Gen[dfH2Gen.Zone.==z,:R_ID],H2_FLEX))
	     	    dfTemp1[t+rowoffset,3] = sum(value.(EP[:vH2_CHARGE_FLEX][y,t]) for y in intersect(dfH2Gen[dfH2Gen.Zone.==z,:R_ID],H2_FLEX))
                dfTemp1[t+rowoffset,4] = -sum(value.(EP[:vH2Gen][dfH2Gen[(dfH2Gen[!,:H2_FLEX].>=1) .&  (dfH2Gen[!,:Zone].==z),:][!,:R_ID],t]))
	     	end
			 if !isempty(intersect(dfH2Gen[dfH2Gen.Zone.==z,:R_ID],H2_STOR_ALL))
				dfTemp1[t+rowoffset,5] = sum(value.(EP[:vH2Gen][y,t]) for y in intersect(dfH2Gen[dfH2Gen.Zone.==z,:R_ID],H2_STOR_ALL))
			   dfTemp1[t+rowoffset,6] = -sum(value.(EP[:vH2_CHARGE_STOR][y,t]) for y in intersect(dfH2Gen[dfH2Gen.Zone.==z,:R_ID],H2_STOR_ALL))
			end

	     	dfTemp1[t+rowoffset,7] = value(EP[:vH2NSE][1,t,z])

			if setup["ModelH2Pipelines"] == 1
			 	dfTemp1[t+rowoffset,8] = value.(EP[:ePipeZoneDemand][t,z])
			else
			...
			if setup["ModelH2Trucks"] == 1
				dfTemp1[t+rowoffset,9] = value.(EP[:eH2TruckFlow][t,z])
			else
