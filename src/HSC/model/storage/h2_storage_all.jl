...
function h2_storage_all(EP::Model, inputs::Dict, setup::Dict)
    ...
    if setup["ParameterScale"] ==1 
        ...
        if (dfH2Gen[!,:H2Stor_Charge_MMBtu_p_tonne][y]>0) # Charging consumes fuel - fuel divided by 1000 since fuel cost already scaled in load_fuels_data.jl when ParameterScale =1
            inputs["omega"][t]*dfH2Gen[!,:Var_OM_Cost_Charge_p_tonne][y]*vH2_CHARGE_STOR[y,t]/ModelScalingFactor^2 + inputs["fuel_costs"][dfH2Gen[!,:Fuel][y]][t] * dfH2Gen[!,:H2Stor_Charge_MMBtu_p_tonne][y]*vH2_CHARGE_STOR[y,t]/ModelScalingFactor
        else
        ...
        if (dfH2Gen[!,:H2Stor_Charge_MMBtu_p_tonne][y]>0) # Charging consumes fuel 
            inputs["omega"][t]*dfH2Gen[!,:Var_OM_Cost_Charge_p_tonne][y]*vH2_CHARGE_STOR[y,t] +inputs["fuel_costs"][dfH2Gen[!,:Fuel][y]][t] * dfH2Gen[!,:H2Stor_Charge_MMBtu_p_tonne][y]
        else
