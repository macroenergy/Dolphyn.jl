...
"""
function write_co2_capture_compression_capacity(path::AbstractString, sep::AbstractString, inputs::Dict, setup::Dict, EP::Model)
	# Capacity decisions
	dfCO2CaptureComp = inputs["dfCO2CaptureComp"]
	capcapture = zeros(size(inputs["CO2_CAPTURE_COMP_NAME"]))
	for i in 1:inputs["CO2_CAPTURE_COMP_ALL"]
		capcapture[i] = value(EP[:vCapacity_CO2_Capture_Comp_per_type][i])
	end

	dfCap = DataFrame(
		Resource = inputs["CO2_CAPTURE_COMP_NAME"], Zone = dfCO2CaptureComp[!,:Zone],
		Capacity = capcapture[:],
	)
	...
	CSV.write(string(path,sep,"CSC_co2_capture_compression_capacity.csv"), dfCap)
	return dfCap
end
