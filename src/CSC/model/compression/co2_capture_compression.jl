...
function co2_capture_compression(EP::Model, inputs::Dict,setup::Dict)
	...
	if setup["ParameterScale"] == 1 # If ParameterScale = 1, power system operation/capacity modeled in GW rather than MW 
		...
		sum(EP[:vPower_CO2_Capture_Comp][k,t]/ModelScalingFactor for k in dfCO2CaptureComp[dfCO2CaptureComp[!,:Zone].==z,:][!,:R_ID]))
	...
	EP[:eDACNetpowerConsumptionByAll] += ePower_Balance_CO2_Capture_Comp

	###############################################################################################################################
	##Constraints
	#Power constraint
	@constraint(EP,cPower_Consumption_CO2_Capture_Comp[k=1:CO2_CAPTURE_COMP_ALL, t = 1:T], EP[:vPower_CO2_Capture_Comp][k,t] == EP[:vCO2_Capture_Compressed][k,t] * dfCO2CaptureComp[!,:etaPCO2_MWh_p_tonne][k])
