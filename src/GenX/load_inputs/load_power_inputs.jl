...
@doc raw"""
	load_power_inputs(setup::Dict,path::AbstractString)
...
function load_power_inputs(inputs::Dict, setup::Dict, path::AbstractString)
    ...
    ## Read input files
    print_and_log("Reading Power Input CSV Files")
    ...
    if setup["OperationWrapping"] == 1 &&
       ...
       (
           isfile(data_directory * "/Period_map.csv") || isfile(
               joinpath(
                   data_directory,
                   string(joinpath(setup["TimeDomainReductionFolder"], "Period_map.csv")),
               ),
           )
       ) # Use Time Domain Reduced data for GenX)
