...
function parse_data(myinputs, mysetup)
    ...
    #Use subperiod results in clustering
    if mysetup["ClusterSubPeriodResults"] == 1

        #Add subperiod power generation results
        for r in 1:length(RESOURCES)
        ...
        if mysetup["ModelH2"] == 1
            for r in 1:length(H2_RESOURCES)
                push!(subperiod_h2_gen_col_names, "H2Gen_$(H2_RESOURCES[r])")
    ...
    parsed_data = Dict(
        ...
        "subperiod_power_gen_col_names" => subperiod_power_gen_col_names,
