...
function write_co2_emission_balance_system(path::AbstractString, inputs::Dict, setup::Dict, EP::Model)
    ...
    for t in 1:T
        ...
        if setup["ModelNGSC"] == 1
            ...
            if setup["ModelH2"] == 1
                ...
                dfTemp1[t+rowoffset,13] = value(sum(EP[:eConv_NG_CO2_Emissions][z,t] for z in 1:Z)) - value(sum(EP[:ePower_NG_CO2_emission_per_zone_per_time][z,t] for z in 1:Z))
            end
    ...
    if setup["ModelNGSC"] == 1
        ...
        if setup["ModelH2"] == 1
            ...
            dfTemp1[rowoffset,13] = sum(sum(inputs["omega"].* (value.(EP[:eConv_NG_CO2_Emissions])[z,:])) for z in 1:Z) - sum(sum(inputs["omega"].* (value.(EP[:ePower_NG_CO2_emission_per_zone_per_time])[z,:])) for z in 1:Z)
        end
