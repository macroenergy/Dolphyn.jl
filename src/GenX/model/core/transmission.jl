...
function transmission(EP::Model, inputs::Dict, UCommit::Int, NetworkExpansion::Int)
    ...
    @expression(
        ...
        sum(abs(inputs["pNet_Map"][l, z]) * vTLOSS[l, t] for l in LOSS_LINES)
    )
    ...
    @expression(EP, ePowerBalanceNetExportFlows[z = 1:Z, t = 1:T], -eNet_Export_Flows[z, t])
    @expression(
        EP,
        ePowerBalanceLossesByZone[z = 1:Z, t = 1:T],
    ...
    if TRANS_LOSS_SEGS == 1
        @constraints(
            ...
            begin
                ...
                vTAUX_POS[l, t] + vTAUX_NEG[l, t] <= eAvail_Trans_Cap[l]
            end
        )
        ...
        if UCommit == 1
            ...
            @constraints(
                ...
                begin
                    ...
                    vTAUX_NEG[l, t] <= eAvail_Trans_Cap[l] - vPROD_TRANSCAP_ON[l, t]
                    ...
                    [l in LOSS_LINES, t = 1:T],
                    vPROD_TRANSCAP_ON[l, t] <= eAvail_Trans_Cap[l]
    ...
    if (TRANS_LOSS_SEGS > 1)
        ...
        @constraint(
            ...
            (
                inputs["pTrans_Loss_Coef"][l] * sum(
                    ...
                    vTAUX_POS[s, l, t] for s = 1:TRANS_LOSS_SEGS
                )
            ) + (
                inputs["pTrans_Loss_Coef"][l] * sum(
                    (2 * s - 1) *
                    (inputs["pTrans_Max_Possible"][l] / TRANS_LOSS_SEGS) *
                    vTAUX_NEG[s, l, t] for s = 1:TRANS_LOSS_SEGS
                )
            )
        )
        ...
        @constraints(
            ...
            begin
                ...
                sum(vTAUX_POS[s, l, t] for s = 1:TRANS_LOSS_SEGS) - vTAUX_POS[0, l, t] ==
                vFLOW[l, t]
                cTAuxSumNeg[l in LOSS_LINES, t = 1:T],
                sum(vTAUX_NEG[s, l, t] for s = 1:TRANS_LOSS_SEGS) - vTAUX_NEG[0, l, t] ==
                -vFLOW[l, t]
            end
        )
        if UCommit == 0 || UCommit == 2
            ...
            @constraints(
                EP,
                begin
                    cTAuxMaxPos[s = 1:TRANS_LOSS_SEGS, l in LOSS_LINES, t = 1:T],
                    vTAUX_POS[s, l, t] <=
                    (inputs["pTrans_Max_Possible"][l] / TRANS_LOSS_SEGS)
                    cTAuxMaxNeg[s = 1:TRANS_LOSS_SEGS, l in LOSS_LINES, t = 1:T],
                    vTAUX_NEG[s, l, t] <=
                    (inputs["pTrans_Max_Possible"][l] / TRANS_LOSS_SEGS)
                end
            )
            ...
            @constraints(
                EP,
                begin
                    cTAuxOrderPos1[s = 1:TRANS_LOSS_SEGS, l in LOSS_LINES, t = 1:T],
                    vTAUX_POS[s, l, t] <=
                    (inputs["pTrans_Max_Possible"][l] / TRANS_LOSS_SEGS) *
                    vTAUX_POS_ON[s, l, s, t]
                    cTAuxOrderNeg1[s = 1:TRANS_LOSS_SEGS, l in LOSS_LINES, t = 1:T],
                    vTAUX_NEG[s, l, t] <=
                    (inputs["pTrans_Max_Possible"][l] / TRANS_LOSS_SEGS) *
                    vTAUX_NEG_ON[s, l, t]
                    cTAuxOrderPos2[s = 1:(TRANS_LOSS_SEGS-1), l in LOSS_LINES, t = 1:T],
                    vTAUX_POS[s, l, t] >=
                    (inputs["pTrans_Max_Possible"][l] / TRANS_LOSS_SEGS) *
                    vTAUX_POS_ON[s+1, l, t]
                    cTAuxOrderNeg2[s = 1:(TRANS_LOSS_SEGS-1), l in LOSS_LINES, t = 1:T],
                    vTAUX_NEG[s, l, t] >=
                    (inputs["pTrans_Max_Possible"][l] / TRANS_LOSS_SEGS) *
                    vTAUX_NEG_ON[s+1, l, t]
                end
            )
            ...
            @constraints(
                ...
                begin
                    ...
                    vTAUX_POS[0, l, t] <=
                    inputs["pTrans_Max_Possible"][l] * (1 - vTAUX_POS_ON[1, l, t])
                    ...
                    vTAUX_NEG[0, l, t] <=
                    inputs["pTrans_Max_Possible"][l] * (1 - vTAUX_NEG_ON[1, l, t])
                end
            )
        end
