...
function h2_production_no_commit_pw(EP::Model, inputs::Dict,setup::Dict)
	...
	sum(EP[:vH2Gen][k,t] for k in intersect(H2_GEN_NO_COMMIT_PW, dfH2Gen[dfH2Gen[!,:Zone].==z,:][!,:R_ID])))
	...
	if setup["ParameterScale"] ==1 # IF ParameterScale = 1, power system operation/capacity modeled in GW rather than MW 
		...
		sum(EP[:vP2G][k,t]/ModelScalingFactor for k in intersect(H2_GEN_NO_COMMIT_PW, dfH2Gen[dfH2Gen[!,:Zone].==z,:][!,:R_ID]))) 
	else # IF ParameterScale = 0, power system operation/capacity modeled in MW so no scaling of H2 related power consumption
		@expression(EP, ePowerBalanceH2GenNoCommit[t=1:T, z=1:Z],
		sum(EP[:vP2G][k,t] for k in intersect(H2_GEN_NO_COMMIT_PW, dfH2Gen[dfH2Gen[!,:Zone].==z,:][!,:R_ID]))) 
	end
    ...
    var_etaP2G_data = inputs["var_etaP2G"]
    # For now build the piecewise represenation directly,
    # but in the future we can give options based on the # of desired segments

	###Constraints###
	# Power and natural gas consumption associated with H2 generation in each time step
    # for (idx, k) in enumerate(H2_GEN_NO_COMMIT_PW)
    #     h2_output = var_etaP2G_data[!,2*idx-1] * dfH2Gen[k, :Cap_Size_tonne_p_hr] # Per-unit output
    #     h2_power = var_etaP2G_data[!,2*idx] .* h2_output # Per-unit power draw
    #     @constraint(EP, [k, t=1:T], EP[:vP2G][k,t] == EP[:vH2GenNewCap][k] * piecewiselinear(EP, EP[:vH2Gen][k,t] / EP[:vH2GenNewCap][k], h2_output, h2_power))
    # end
	set_optimizer_attribute(EP, "NonConvex", 2)

	@variable(EP, 0 <= vH2UnitGenFrac[k in H2_GEN_NO_COMMIT_PW, t=1:T] <= 1)
	for (idx, k) in enumerate(H2_GEN_NO_COMMIT_PW)
        h2_output = var_etaP2G_data[!,2*idx-1] * dfH2Gen[k, :Cap_Size_tonne_p_hr] # Per-unit output
        h2_power = var_etaP2G_data[!,2*idx] .* h2_output # Per-unit power draw
        # @expression(EP, eH2UnitPower[k, t=1:T], piecewiselinear(EP, EP[:vH2UnitGenFrac][k,t] * dfH2Gen[k, :Cap_Size_tonne_p_hr], h2_output, h2_power))
	...
	# for (idx, k) in enumerate(H2_GEN_NO_COMMIT_PW)
	# 	unit_h2_output = var_etaP2G_data[!,2*idx-1] * dfH2Gen[k, :Cap_Size_tonne_p_hr]
	# 	num_units = EP[:eH2GenTotalCap][k]
	# 	h2_power = var_etaP2G_data[!,2*idx] .* unit_h2_output * num_units
	# 	@constraint(EP, [k, t=1:T], EP[:vP2G][k,t] == piecewiselinear(EP, EP[:vH2Gen][k,t], EP[:eH2GenTotalCap][k], unit_h2_output, 0:10:100, h2_power))

	
	@constraints(EP, begin
	# Maximum power generated per technology "k" at hour "t"
	[k in H2_GEN_NO_COMMIT_PW, t=1:T], EP[:vH2Gen][k,t] <= EP[:eH2GenTotalCap][k]* inputs["pH2_Max"][k,t]
	end)
	...
	@constraints(EP, begin
		...
		[k in H2_GEN_NO_COMMIT_PW, t in START_SUBPERIODS], EP[:vH2Gen][k,t]-EP[:vH2Gen][k,(t + hours_per_subperiod-1)] <= dfH2Gen[!,:Ramp_Up_Percentage][k] * EP[:eH2GenTotalCap][k]

		# Interior Hours
		[k in H2_GEN_NO_COMMIT_PW, t in INTERIOR_SUBPERIODS], EP[:vH2Gen][k,t]-EP[:vH2Gen][k,t-1] <= dfH2Gen[!,:Ramp_Up_Percentage][k]*EP[:eH2GenTotalCap][k]

		## Maximum ramp down between consecutive hours
		# Start Hours: Links last time step with first time step, ensuring position in hour 1 is within eligible ramp of final hour position
		[k in H2_GEN_NO_COMMIT_PW, t in START_SUBPERIODS], EP[:vH2Gen][k,(t+hours_per_subperiod-1)] - EP[:vH2Gen][k,t] <= dfH2Gen[!,:Ramp_Down_Percentage][k] * EP[:eH2GenTotalCap][k]

		# Interior Hours
		[k in H2_GEN_NO_COMMIT_PW, t in INTERIOR_SUBPERIODS], EP[:vH2Gen][k,t-1] - EP[:vH2Gen][k,t] <= dfH2Gen[!,:Ramp_Down_Percentage][k] * EP[:eH2GenTotalCap][k]
