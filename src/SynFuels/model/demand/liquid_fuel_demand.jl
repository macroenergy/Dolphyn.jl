...
function liquid_fuel_demand(EP::Model, inputs::Dict, setup::Dict)
    ...
    if setup["AllowConventionalDiesel"] == 1
        ...
        if setup["BIO_Diesel_On"] == 0
            if setup["SpecifySynBioDieselPercentFlag"] == 1
                ...
                @constraint(EP, cSynFuelDieselShare, (percent_sbf_diesel - 1) * eSynFuelProdNoCommit_DieselTZ + percent_sbf_diesel *  eConvLFDieselDemandTZ == 0)
            end
        end 
    end
    ...
    if setup["AllowConventionalJetfuel"] == 1
        ...
        if setup["SpecifySynJetfuelPercentFlag"] == 1
            ...
            @expression(EP, eSynFuelProdNoCommit_JetfuelT[t=1:T], sum(EP[:eSynFuelProdNoCommit_Jetfuel][t, z] for z in 1:Z))
            @expression(EP, eSynFuelProdNoCommit_JetfuelTZ, sum(eSynFuelProdNoCommit_JetfuelT[t] for t in 1:T))
        
            @constraint(EP, cSynFuelJetfuelShare, (percent_sf_jetfuel - 1) * eSynFuelProdNoCommit_JetfuelTZ + percent_sf_jetfuel *  eConvLFJetfuelDemandTZ == 0)
    ...
    if setup["AllowConventionalGasoline"] == 1
        ...
        if setup["BIO_Gasoline_On"] == 0
            if setup["SpecifySynBioGasolinePercentFlag"] == 1
                ...
                @constraint(EP, cSynFuelGasolineShare, (percent_sbf_gasoline - 1) * eSynFuelProdNoCommit_GasolineTZ + percent_sbf_gasoline *  eConvLFGasolineDemandTZ == 0)
            end
        end 
    end
