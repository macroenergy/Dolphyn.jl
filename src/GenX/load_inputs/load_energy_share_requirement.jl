...
function load_energy_share_requirement(setup::Dict, path::AbstractString, sep::AbstractString, inputs_ESR::Dict)
	...
	dfESR = DataFrame(CSV.File(string(path, sep,"Energy_share_requirement.csv"), header=true), copycols=true)
	
	# Filter energy share requirements by zone
	dfESR = filter(row -> row.Network_zones in Zones, dfESR)

	inputs_ESR["dfESR"] = dfESR
