...
function co2_capture_compression_investment(EP::Model, inputs::Dict, setup::Dict)
	...
	if setup["ParameterScale"] ==1 
		RefCAPEX_per_t_per_h_y = dfCO2CaptureComp[!,:RefCAPEX_per_y]/ModelScalingFactor^2 # $/t/h/y
		RefFixed_OM_per_t_per_h_y = dfCO2CaptureComp[!,:RefFixedOM_per_y]/ModelScalingFactor^2 # $/t/h/y
	else
		RefCAPEX_per_t_per_h_y = dfCO2CaptureComp[!,:RefCAPEX_per_y] # $/t/h/y
		RefFixed_OM_per_t_per_h_y = dfCO2CaptureComp[!,:RefFixedOM_per_y] # $/t/h/y
	end

	#Load capacity parameters
	RefCapacity_t_per_h = dfCO2CaptureComp[!,:RefCapacity_Mt_y]/(365*24)*10^6 # t/h
	CO2_Capture_Comp_Capacity_Min_Limit = dfCO2CaptureComp[!,:MinCapacity_Mt_y]/(365*24)*10^6 # t/h
	CO2_Capture_Comp_Capacity_Max_Limit = dfCO2CaptureComp[!,:MaxCapacity_Mt_y]/(365*24)*10^6 # t/h
	...
	if setup["CO2_Capture_Comp_CAPEX_Piecewise"] ==1 
		#####################################################################################################################################
	...
	elseif setup["CO2_Capture_Comp_CAPEX_Piecewise"] == 0
