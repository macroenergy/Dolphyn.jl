...
function co2_injection(EP::Model, inputs::Dict,setup::Dict)
	...
	if setup["ParameterScale"] == 1 # If ParameterScale = 1, power system operation/capacity modeled in GW rather than MW 
		...
		sum(EP[:vPower_CO2_Injection][k,t]/ModelScalingFactor for k in dfCO2Storage[dfCO2Storage[!,:Zone].==z,:][!,:R_ID]))
	...
	EP[:eDACNetpowerConsumptionByAll] += ePower_Balance_CO2_Injection
	...
	##Constraints
	#Power constraint
	@constraint(EP,cPower_Consumption_CO2_Injection[k=1:CO2_STOR_ALL, t = 1:T], EP[:vPower_CO2_Injection][k,t] == EP[:vCO2_Injected][k,t] * dfCO2Storage[!,:etaPCO2_MWh_p_tonne][k])
