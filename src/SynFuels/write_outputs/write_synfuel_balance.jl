...
function write_synfuel_balance(path::AbstractString, sep::AbstractString, inputs::Dict, setup::Dict, EP::Model)
	...
	for z in 1:Z
		...
		byprodHead = "ByProd_Out_" .* string.(collect(1:NSFByProd))
	   	dfTemp1[1,1:size(dfTemp1,2)] = vcat(["CO2_In","Power_In", "H2_In","Syn_Fuel_Out"], byprodHead)
	   	...
	   	for t in 1:T
			...
			dfTemp1[t+rowoffset,4]= sum(value.(EP[:vSFProd][dfSynFuels[(dfSynFuels[!,:Zone].==z),:][!,:R_ID],t]))

			for b in 1:NSFByProd
				dfTemp1[t+rowoffset, 4 + b] = sum(value.(EP[:vSFByProd][dfSynFuels[(dfSynFuels[!,:Zone].==z),:][!,:R_ID],b,t]))
			end
