...
Instead of modeling capacity reserve margin requirement (a.k.a. capacity market or resource adequacy requirement) using an annual constraint, we model each requirement with hourly constraint by simulating the activation of the capacity obligation.
We define capacity reserve margin constraint for subsets of zones, $z \in \mathcal{Z}^{CRM}_{p}$, and each subset stands for a locational deliverability area (LDA) or a reserve sharing group.
For thermal resources, the available capacity is the total capacity in the LDA derated by the outage rate, $\epsilon_{k,z,p}^{CRM}$.
For storage ($s \in \mathcal{S}$), variable renewable energy ($r \in \mathcal{VRE}$), and flexibile demand resources ($y \in \mathcal{DF}$), the available capacity is the net injection into the transmission network in time step $t$ derated by the derating factor, also stored in the parameter, $\epsilon_{y,z,p}^{CRM}$.
If the imported capacity is eligible to provide capacity to the CRM constraint, the inbound powerflow on all lines $\mathcal{L}_{p}^{in}$ in time step $t$ will be derated to form the available capacity from outside of the LDA.
The reverse is true as well: the outbound derated powerflow on all lines $\mathcal{L}_{p}^{out}$ in time step $t$ is taken out from the total available capacity.
...
We allow all segments of voluntary load curtailment, $s \geq 2 \in \mathcal{SEG}$, to contribute to capacity requirements. The first segment $s = 1 \in \mathcal{SEG}$ corresponds to involuntary demand curtailment or emergency load shedding at the price cap or value of lost load, and thus does not contribute to reserve requirements.
Note that the time step-weighted sum of the shadow prices of this constraint corresponds to the capacity market payments reported by ISOs with mandate capacity market mechanism.
...
%\end{aligned}
\begin{aligned}
   & \sum_{z  \in \mathcal{Z}^{CRM}_{p}} \Big( \sum_{y \in \mathcal{K}} \epsilon_{y,z,p}^{CRM} \times y_{k}^{\textrm{E, THE}} + \sum_{y \in \mathcal{VRE}} \epsilon_{y,z,p}^{CRM} \times y_r^{\textrm{E, VRE}} \\
   + & \sum_{y \in \mathcal{S}} \epsilon_{y,z,p}^{CRM} \times \left(y_s^{\textrm{E, STO, DIS}} - y_s^{\textrm{E, STO, CHA}} \right) + \sum_{y \in \mathcal{DF}} \epsilon_{y,z,p}^{CRM} \times x_{y,z,p}^{\textrm{E, FLEX}} \right) \\
   + & \sum_{l \in \mathcal{L}_{p}^{in}} \epsilon_{y,z,p}^{CRM} \times \Phi_{l,t} -  \sum_{l \in \mathcal{L}_{p}^{out}} \epsilon_{y,z,p}^{CRM} \times \Phi_{l,t}
   +  \sum_{s \geq 2} \Lambda_{s,t,z}  \Big) \\
   & \geq \sum_{z  \in \mathcal{Z}^{CRM}_{p}} \left( \left(1 + RM_{z,p}^{CRM} \right) \times D_{z,t} \right)  \hspace{1 cm}  \forall t \in \mathcal{T}, \forall p\in \mathcal{P}^{CRM}
\end{aligned}
...
function cap_reserve_margin(EP::Model, inputs::Dict, setup::Dict)
	...
	print_and_log("Capacity Reserve Margin Policies Module")
	dfGen = inputs["dfGen"]
	G = inputs["G"]
