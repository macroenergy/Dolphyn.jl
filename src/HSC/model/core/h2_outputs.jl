...
function h2_outputs(EP::Model, inputs::Dict, setup::Dict)
    ...
    #H2 injected to hydrogen grid from hydrogen generation resource k (MW of H2/hr) in time t
    ...
    if setup["ParameterScale"] ==1
        ...
        (inputs["omega"][t] * (dfH2Gen[!,:Var_OM_Cost_p_MWh][k]/ModelScalingFactor^2 + inputs["fuel_costs"][dfH2Gen[!,:Fuel][k]][t] * dfH2Gen[!,:etaFuel_MMBtu_p_MWh][k]/ModelScalingFactor) * vH2Gen[k,t]))
    else
        @expression(EP, eCH2GenVar_out[k = 1:H,t = 1:T], 
        (inputs["omega"][t] * ((dfH2Gen[!,:Var_OM_Cost_p_MWh][k] + inputs["fuel_costs"][dfH2Gen[!,:Fuel][k]][t] * dfH2Gen[!,:etaFuel_MMBtu_p_MWh][k])) * vH2Gen[k,t]))
    end
