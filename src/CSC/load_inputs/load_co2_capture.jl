function load_co2_capture(setup::Dict, path::AbstractString, sep::AbstractString, inputs_capture::Dict)
	...
	inputs_capture["CO2_CAPTURE_SOLID"] = co2_capture[co2_capture.CO2_CAPTURE_TYPE.==1 ,:R_ID]

	# Set of CO2 resources not eligible for unit committment
	inputs_capture["CO2_CAPTURE_LIQUID"] = co2_capture[co2_capture.CO2_CAPTURE_TYPE.==2 ,:R_ID]

    #Set of all CO2 capture Units - can be either commit or new commit
    inputs_capture["CO2_CAPTURE"] = union(inputs_capture["CO2_CAPTURE_SOLID"],inputs_capture["CO2_CAPTURE_LIQUID"])
