...
"""
function write_co2_capture(path::AbstractString, sep::AbstractString, inputs::Dict, setup::Dict, EP::Model)
	...
	for i in 1:H
		dfCO2CaptureOut[!,:AnnualSum][i] = sum(inputs["omega"].* (value.(EP[:vCO2Capture])[i,:]))
	end
	# Load hourly values
	...
	dfPower = vcat(dfCO2CaptureOut, total)
 	...
 	CSV.write(string(path,sep,"CSC_co2_capture_discharge.csv"), dftranspose(dfCO2CaptureOut, false), writeheader=false)
