...
@doc raw"""
	load_h2_gen(path::AbstractString, setup::Dict, inputs::Dict)
...
function load_h2_gen(path::AbstractString, setup::Dict, inputs::Dict)
	...
	inputs["H2_RESOURCE_ZONES"] = inputs["H2_RESOURCES_NAME"] .* "_z" .* string.(h2_zones)

	# Set of flexible demand-side resources
	inputs["H2_FLEX"] = h2_gen_in[h2_gen_in.H2_FLEX.==1,:R_ID]

	# To do - will add a list of storage resources or we can keep them separate
	# Set of H2 storage resources
	inputs["H2_STOR_SYMMETRIC"] = h2_gen_in[h2_gen_in.H2_STOR.==1,:R_ID]
	inputs["H2_STOR_ASYMMETRIC"] = h2_gen_in[h2_gen_in.H2_STOR.==2,:R_ID]
	inputs["H2_STOR_ALL"] = union(inputs["H2_STOR_SYMMETRIC"],inputs["H2_STOR_ASYMMETRIC"])
	...
	inputs["H2_STOR_LONG_DURATION"] = h2_gen_in[(h2_gen_in.LDS.==1) .& (h2_gen_in.H2_STOR.==1),:R_ID]
	inputs["H2_STOR_SHORT_DURATION"] = h2_gen_in[(h2_gen_in.LDS.==0) .& (h2_gen_in.H2_STOR.==1),:R_ID]
	...
	inputs["RET_CAP_H2_ENERGY"] = intersect(h2_gen_in[h2_gen_in.New_Build.!=-1,:R_ID], h2_gen_in[h2_gen_in.Existing_Energy_Cap_tonne.>0,:R_ID], inputs["H2_STOR_ALL"])
	...
	inputs["H2_GEN_NO_COMMIT"] = intersect(h2_gen_in[h2_gen_in.H2_GEN_TYPE.==2 ,:R_ID], h2_gen_in[h2_gen_in.H2_FLEX.!=1 ,:R_ID])

    #Set of all H2 production Units - can be either commit or new commit
    inputs["H2_GEN"] = union(inputs["H2_GEN_COMMIT"],inputs["H2_GEN_NO_COMMIT"])
	...
	# 	inputs["dfH2Gen"][!,:CO2_per_tonne][g] =inputs["fuel_CO2"][dfH2Gen[!,:Fuel][k]][t] * dfH2Gen[!,:etaFuel_MMBtu_p_tonne][k]))
