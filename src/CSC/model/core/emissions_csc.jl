...
function emissions_csc(EP::Model, inputs::Dict, setup::Dict)
    ...
    K = inputs["CO2_RES_ALL"]     # Number of resources (generators, storage, flexible demand)
    ...
    @expression(
        EP,
        eCO2NegativeEmissionsByPlant[k in 1:K, t = 1:T],
        EP[:vCO2Capture][k, t] * (
            1 -
            inputs["fuel_CO2"][dfCO2Capture[!, :Fuel][k]] *
            dfCO2Capture[!, :etaFuel_MMBtu_p_tonne][k]
        )
    )

    @expression(
        EP,
        eCO2NegativeEmissionsByZone[z = 1:Z, t = 1:T],
        sum(
            eCO2NegativeEmissionsByPlant[y, t] for
            y in dfCO2Capture[(dfCO2Capture[!, :Zone].==z), :R_ID]
        )
    )
    ...
    if (setup["CO2CostOffset"] == 1 && setup["SystemCO2Constraint"] == 1)
        ...
        @expression(
            EP,
            eCCO2EmissionsOffsetbyZone[z = 1:Z],
            sum(
                inputs["omega"][t] *
                eCO2NegativeEmissionsByZone[z, t] *
                inputs["dfCO2CO2PriceZone"][z] for t = 1:T
            )
        )
        ...
        @expression(
            EP,
            eCCO2CaptureTotalEmissionsOffset,
    ...
    elseif (setup["CO2Cap"] == 4 && setup["SystemCO2Constraint"] == 2)
        ...
        @expression(
            ...
            sum(
                inputs["omega"][t] * sum(
                    eCO2NegativeEmissionsByZone[z, t] * inputs["dfCO2Price"][z, cap] for
