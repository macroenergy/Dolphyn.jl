...
function write_co2_storage_balance_zone(path::AbstractString, sep::AbstractString, inputs::Dict, setup::Dict, EP::Model)
	...
	dfCost = DataFrame(Costs = ["Power_CCS", "H2_CCS", "DAC_Capture", "DAC_Fuel_CCS", "Biorefinery_Capture", "Synfuel_Production_Capture", "Synfuel_Production_Consumption", "SNG_Production_Capture", "SNG_Production_Consumption", "CO2_Pipeline_Import", "CO2_Storage", "Total"])
	...
	SNG_Production_Capture = 0
	SNG_Production_Consumption = 0

	#if setup["ModelNGSC"] == 1 && setup["ModelSyntheticNG"] == 1
	#	Synfuel_Production_Capture = sum(sum(inputs["omega"].* (value.(EP[:eSyn_NG_CO2_Capture_Per_Zone_Per_Time])[z,:])) for z in 1:Z)
	#	Synfuel_Production_Consumption = - sum(sum(inputs["omega"].* (value.(EP[:eSyn_NG_CO2_Cons_Per_Zone_Per_Time])[z,:])) for z in 1:Z)
	#end
	...
	cTotal = Power_CCS + H2_CCS + DAC_Capture + DAC_Fuel_CCS + Biorefinery_Capture + Synfuel_Production_Capture + Synfuel_Production_Consumption + SNG_Production_Capture + SNG_Production_Consumption + CO2_Pipeline_Import + CO2_Storage
	...
	if setup["ParameterScale"] == 1
		...
		SNG_Production_Capture = Synfuel_Production_Capture * ModelScalingFactor
		SNG_Production_Consumption = Synfuel_Production_Consumption * ModelScalingFactor
	...
	dfCost[!,Symbol("Total")] = [Power_CCS, H2_CCS, DAC_Capture, DAC_Fuel_CCS, Biorefinery_Capture, Synfuel_Production_Capture, Synfuel_Production_Consumption, SNG_Production_Capture, SNG_Production_Consumption, CO2_Pipeline_Import, CO2_Storage, cTotal]
	...
	for z in 1:Z
		...
		tempSNG_Production_Capture = 0
		tempSNG_Production_Consumption = 0
		...
		#if setup["ModelNGSC"] == 1 && setup["ModelSyntheticNG"] == 1
		#	tempSNG_Production_Capture = tempSNG_Production_Capture + sum(inputs["omega"].* (value.(EP[:eSyn_NG_CO2_Capture_Per_Zone_Per_Time])[z,:]))
		#	tempSNG_Production_Consumption = tempSNG_Production_Consumption - sum(inputs["omega"].* (value.(EP[:eSyn_NG_CO2_Cons_Per_Zone_Per_Time])[z,:]))
		#end
		...
		tempCTotal = tempPower_CCS + tempH2_CCS + tempDAC_Capture + tempDAC_Fuel_CCS + tempBiorefinery_Capture + tempSynfuel_Production_Capture + tempSynfuel_Production_Consumption + tempSNG_Production_Capture + tempSNG_Production_Consumption + tempCO2_Pipeline_Import + tempCO2_Storage
		...
		if setup["ParameterScale"] == 1
			...
			tempSNG_Production_Capture = tempSNG_Production_Capture * ModelScalingFactor
			tempSNG_Production_Consumption = tempSNG_Production_Consumption * ModelScalingFactor
		...
		dfCost[!,Symbol("Zone$z")] = [tempPower_CCS, tempH2_CCS, tempDAC_Capture, tempDAC_Fuel_CCS, tempBiorefinery_Capture, tempSynfuel_Production_Capture, tempSynfuel_Production_Consumption, tempSNG_Production_Capture,  tempSNG_Production_Consumption,  tempCO2_Pipeline_Import, tempCO2_Storage, tempCTotal]
	end
