...
function write_co2_capture_costs(path::AbstractString, sep::AbstractString, inputs::Dict, setup::Dict, EP::Model)
	...
	CO2_CAPTURE_UC = inputs["CO2_CAPTURE_UC"] #DAC with unit commitment
	...
	CSV.write(string(path,sep,"CSC_DAC_total_costs.csv"), dfDACCost)
	...
	TotalCost = zeros(size(inputs["CO2_RESOURCES_NAME"]))

	for i in 1:inputs["CO2_RES_ALL"]
		
		if setup["ParameterScale"]==1
			CAPEX[i] = value(EP[:eCAPEX_DAC_per_type][i])*ModelScalingFactor^2
			FixedOM[i] = value(EP[:eFixed_OM_DAC_per_type][i])*ModelScalingFactor^2
			VarOM[i] = value(EP[:eVar_OM_DAC_per_type][i])*ModelScalingFactor^2
		else
			CAPEX[i] = value(EP[:eCAPEX_DAC_per_type][i])
			FixedOM[i] = value(EP[:eFixed_OM_DAC_per_type][i])
			VarOM[i] = value(EP[:eVar_OM_DAC_per_type][i])
		end

		Startup[i] = 0

		if i in CO2_CAPTURE_UC
			if setup["ParameterScale"]==1
				Startup[i] = value(EP[:eTotal_Startup_Cost_DAC_per_type][i])*ModelScalingFactor^2
			else
				Startup[i] = value(EP[:eTotal_Startup_Cost_DAC_per_type][i])
			end
		end
		TotalCost[i] = CAPEX[i] + FixedOM[i] + VarOM[i] + Startup[i]
	end
	...
	CSV.write(string(path,sep,"CSC_DAC_Plant_Cost.csv"), dfCost_Plant)
